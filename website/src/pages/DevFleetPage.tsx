...
function ConfirmBtn({ title, desc, confirmLabel, onConfirm, btn, children }: ConfirmBtnProps) {
  ...
  return (
    <span style={{ display: 'inline-flex' } as CSSProperties}>
      <Btn ref={triggerRef} {...(btn || {})} onClick={toggle}>{children}</Btn>
      {open && rect && createPortal(
        <div ref={popRef} style={{ ...posStyle, zIndex: 4000, background: 'var(--card, #16161a)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 12px', width: 264, boxShadow: '0 8px 24px rgba(0,0,0,0.45)', textAlign: 'left' as const } as CSSProperties}>
