        ...
        <div role="tablist" className="inline-flex rounded border border-border overflow-hidden">
          {(
            ...
            <button
              ...
              className={`px-3 py-1 text-[12px] cursor-pointer border-r border-border last:border-r-0 ${
                view === v ? 'bg-bg-elevated text-text-strong font-medium' : 'text-muted hover:text-text'
              }`}
            >
              {label}
            </button>
              ...
              {rows.map(r => (
                ...
                >
                  <td
                    ...
                    title={r.name}
                  >
                    {/* Presentational, and deliberately so: rows start expanded,
                        so collapsing only hides what is already on screen. The
                        row itself is the navigable control (role=button); a real
                        nested button inside it would be invalid interactive
                    ...
                    {r.hasTasks && (
                      <span
                        ...
                        className="inline-block w-3 -ml-3 mr-0.5 text-muted hover:text-text cursor-pointer select-none"
                      >
                        {r.expanded ? '▾' : '▸'}
                      </span>
                    )}
