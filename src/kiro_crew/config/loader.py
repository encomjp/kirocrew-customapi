...
class AgentConfig:
    ...
    def resolve_effort(self, role: str) -> str:
        """Effective reasoning effort for a task ``role``.

        Chain (most-specific first; ``""`` defers down): ``role_efforts[role]``
        -> ``reasoning_effort`` (interactive chat default) -> ``""`` (the
        provider/model default). Effort only takes effect on reasoning-capable
        models; on others the resolved value is ignored downstream.
        """
        pin = self.role_efforts.get(role, "")
        if pin:
            return pin
