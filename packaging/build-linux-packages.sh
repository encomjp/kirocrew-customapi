#!/bin/bash
# Build KiroCrew-9Router Linux packages: .deb, .rpm, relocatable tarball.
#
# Produces (in dist/packages/):
#   kirocrew-9router_<ver>_amd64.deb
#   kirocrew-9router-<ver>-1.x86_64.rpm
#   kirocrew-9router-<ver>-linux-x86_64.tar.gz
#
# Each package bundles a self-contained Python venv (/opt/kirocrew-9router)
...
#!/bin/sh
exec /opt/kirocrew-9router/venv/bin/kirocrew "$@"
EOF
chmod 755 "$STAGE/usr/bin/kirocrew"

cat > "$STAGE/usr/share/applications/kirocrew-9router.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=Kiro Crew
...
Homepage: https://github.com/encomjp/KiroCrew-9Router
Description: Kiro Crew with the Claude Code ACP backend for custom LLM routers
 ...
 (e.g. a local 9router instance speaking the Anthropic API) instead of
...
# ---- tarball -------------------------------------------------------------
TARBALL="$(mktemp -d)"
mkdir -p "$TARBALL/kirocrew-9router-$VERSION"
cp -a "$STAGE/opt/$PKG/venv" "$TARBALL/kirocrew-9router-$VERSION/"
cat > "$TARBALL/kirocrew-9router-$VERSION/install.sh" <<'EOF'
#!/bin/sh
# KiroCrew-9Router installer - relocatable tarball install
# Usage: ./install.sh [--prefix /opt/kirocrew-9router] [--user]
set -e
PREFIX="/opt/kirocrew-9router"
MODE="system"
case "$1" in
  --user) PREFIX="$HOME/.local/share/kirocrew-9router"; MODE="user" ;;
  --prefix) PREFIX="$2"; shift ;;
esac
echo "==> Installing KiroCrew-9Router to $PREFIX"
mkdir -p "$PREFIX"
cp -a venv "$PREFIX/"
...
EOF
chmod 755 "$TARBALL/kirocrew-9router-$VERSION/install.sh"
tar -czf "$DIST/kirocrew-9router-$VERSION-linux-x86_64.tar.gz" \
    -C "$TARBALL" "kirocrew-9router-$VERSION"
echo "==> tarball: $DIST/kirocrew-9router-$VERSION-linux-x86_64.tar.gz"
...
TARBALL2="$(mktemp -d)"
mkdir -p "$TARBALL2/kirocrew-9router-$VERSION/opt"
cp -a "$STAGE/opt/$PKG" "$TARBALL2/kirocrew-9router-$VERSION/opt/"
cp "$STAGE/usr/share/applications/kirocrew-9router.desktop" \
   "$STAGE/usr/share/icons/hicolor/512x512/apps/kirocrew.png" \
   "$TARBALL2/kirocrew-9router-$VERSION/" 2>/dev/null || true
tar -czf "$RPMTARBALL" -C "$TARBALL2" "kirocrew-9router-$VERSION"
...
License:        Apache-2.0
URL:            https://github.com/encomjp/KiroCrew-9Router
...
%install
mkdir -p %{buildroot}/opt/kirocrew-9router
mkdir -p %{buildroot}/usr/bin
mkdir -p %{buildroot}/usr/share/applications
mkdir -p %{buildroot}/usr/share/icons/hicolor/512x512/apps
cp -a opt/kirocrew-9router/venv %{buildroot}/opt/kirocrew-9router/
cat > %{buildroot}/usr/bin/kirocrew <<'INNEREOF'
#!/bin/sh
exec /opt/kirocrew-9router/venv/bin/kirocrew "\$@"
INNEREOF
chmod 755 %{buildroot}/usr/bin/kirocrew
cp kirocrew-9router.desktop %{buildroot}/usr/share/applications/
...
%files
/opt/kirocrew-9router/
/usr/bin/kirocrew
/usr/share/applications/kirocrew-9router.desktop
...
- Initial package: KiroCrew-9Router with bundled venv
EOF
